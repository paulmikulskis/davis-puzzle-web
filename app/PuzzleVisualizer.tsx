...
export function PuzzleVisualizer({
  ...
  return (
    <section className="davis-card-enter rounded-lg border border-[var(--border)] bg-white p-4 shadow-sm sm:p-5">
      <div className="flex flex-col gap-4 border-b border-[var(--border)] pb-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex w-full shrink-0 flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center">
          <button
            ...
            className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-md bg-[var(--accent)] px-5 py-3.5 text-base font-semibold text-white shadow-sm transition hover:bg-[var(--accent-hover)] hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[var(--accent-soft)] focus:ring-offset-2"
          >
          ...
          <button
            ...
            className="cursor-pointer rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm font-medium text-[var(--heading)] transition hover:border-[var(--accent)] hover:bg-[var(--panel)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-soft)] focus:ring-offset-2"
          >
          ...
          {emailState.status === "success" ? (
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                type="button"
                onClick={resetEmail}
                className="cursor-pointer self-start rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm font-medium text-[var(--heading)] transition hover:border-[var(--accent)] hover:bg-[var(--panel)] sm:self-auto"
              >
                Done
              </button>
            </div>
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="mt-2 w-full resize-y rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm leading-5 outline-none transition hover:border-[var(--heading)] focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent-soft)] disabled:cursor-not-allowed disabled:bg-slate-100"
                />
              </div>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={resetEmail}
                    disabled={emailState.status === "sending"}
                    className="cursor-pointer rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm font-medium text-[var(--heading)] transition hover:border-[var(--accent)] hover:bg-[var(--panel)] disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={emailState.status === "sending"}
                    className="cursor-pointer rounded-md bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[var(--accent-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-soft)] focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-slate-400"
                  >
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="davis-filter-row -mx-1 flex items-center gap-2 overflow-x-auto px-1 pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0">
          {slides.map((slide, index) => (
            <button
              ...
              className={`shrink-0 cursor-pointer rounded-md border px-3 py-2 text-sm font-medium transition ${
                activeSlide === index
        ...
        <div className="flex shrink-0 gap-2">
          <button
            type="button"
            onClick={goPrevious}
            className="cursor-pointer rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm font-semibold text-[var(--heading)] transition hover:border-[var(--accent)] hover:bg-[var(--panel)]"
          >
            Previous
          </button>
          <button
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--border)] bg-white px-4 py-3 shadow-[0_-4px_16px_rgba(15,23,42,0.06)] sm:hidden">
        <div className="flex items-center gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-[11px] text-[var(--muted)]">
              {puzzle.opaqueCellCount} cells · {puzzle.colorCount} colors
            </p>
          </div>
          <button
            type="button"
            onClick={() => onConfirm(puzzle.url, puzzle.filename)}
            className="inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 rounded-md bg-[var(--accent)] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[var(--accent-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-soft)] focus:ring-offset-2"
          >
            <span aria-hidden="true">↓</span>
            Download
          </button>
      ...
      <div className="flex items-baseline justify-between gap-2">
        ...
        {pinnedSwatch !== null ? (
          <button
            type="button"
            onClick={onClearPin}
            className="cursor-pointer rounded-md border border-[var(--border)] bg-white px-2 py-1 text-[11px] font-medium text-[var(--heading)] transition hover:border-[var(--accent)] hover:bg-[var(--panel)]"
          >
            Clear pin
          </button>
        ) : (
          <span className="text-[11px] font-medium text-[var(--muted)]">
            <span className="davis-pointer-only">
              Hover to glow · click to pin
            </span>
            <span className="davis-touch-only">Tap a swatch to pin</span>
          </span>
        )}
      </div>
        ...
        {palette.map((entry, index) => {
          ...
          return (
            <button
              ...
              className={`flex w-full cursor-pointer gap-3 rounded-md border p-2.5 text-left transition ${
                isPinned
      ...
      <div className="flex items-center gap-3 text-xs text-[var(--muted)]">
        ...
        {!isDefault ? (
          <button
            ...
            className="cursor-pointer rounded-md border border-[var(--border)] bg-white px-2 py-1 font-medium text-[var(--heading)] transition hover:border-[var(--accent)] hover:bg-[var(--panel)]"
          >
            Reset to A-P / a-p
          </button>
        ) : null}
        ...
        {options.map((option) => {
          ...
          return (
            <button
              ...
              className={`cursor-pointer px-3.5 py-2 text-xs font-semibold transition ${
                isActive
